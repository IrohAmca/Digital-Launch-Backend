...
router.post('/update-and-create-general', async (req, res) => {
    try {
        ...
        if (req.body.id) {
            await (0, inserstData_1.updateSection)("LaunchFormData", req.body.LaunchFormData, req.body.id);
            res.status(200).send("Updated General Info");
        }
        else {
            const objID = await (0, inserstData_1.submitGeneral)(req.body);
...
router.post('/insert-placement', async (req, res) => {
    try {
        ...
        if (req.body.id) {
            await (0, inserstData_1.updateSection)("Placements.Placement", req.body.Placement, req.body.id);
